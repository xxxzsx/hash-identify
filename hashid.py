...
def hashid(hash):
    ...
    if len(hash) == len(hs) and hash.isdigit() == False and hash.isalpha() == False and hash.isalnum() == True:
        possibles.append("122080")


print(logo)
try: passed = str(argv[1])
except: passed = None
...
while True:
    try:
        ...
        if len(possibles) == 0:
            ...
            possibles.sort()
            print("\nPossible Hashes:")
            for a in range(len(possibles)):
                if a == 2: print("\nLeast possible Hashes:")
                print("[+] " + str(algorithms[possibles[a]]))
